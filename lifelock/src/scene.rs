...
pub struct Scene {
    ...
    pub validating: bool,
    pub caps_lock: bool,
    /// Mirrored from Machine each tick; rendered as "N FAILED" under the date
    /// (swaylock's show-failed-attempts parity).
    pub failed_attempts: u32,
    /// PAM conversation text from the last failed attempt (e.g. pam_faillock's
    /// "account locked" notice) — rendered under the FAILED line until the
    /// next submit. None for a plain wrong password.
